...
class SBOLRenderer(dpl.DNARenderer):
    ...
    def renderSBOL(
        ...
        sbol_design = (
            []
        )  # Contains a list of DNA components corresponding to the items in dpl_design
        try:
            current_ann = target_component.annotations[0]
        except:
            print(
                "Target DNAComponent does not have any SequenceAnnotations.  Cannot render SBOL."
            )
        END_OF_DESIGN = False
        while not END_OF_DESIGN:
            try:
                ...
                print(
                    "DNAComponent does not have subcomponents.  Cannot render SBOL."
                )
            ...
            if len(current_ann.precedes) == 0:
                ...
                current_ann = current_ann.precedes[
                    0
                ]  # Iterate to the next downstream annotation

        start, end = self.renderDNA(
            ax, dpl_design, part_renderers, plot_backbone=plot_backbone
        )
        return start, end
