        ...
        if len(row[0]) != "":
            ...
            for i in range(1, len(row)):
                ...
                if len(part_name) != 0:
                    ...
                    part_design[
                        "name"
                    ] = part_name  # needed to add part name for regulation
...
def load_regulatory_information(filename, part_info, dna_designs):
    ...
    attrib_keys = [
        ...
        if k not in ["from_partname", "type", "to_partname"]
    ]
        ...
        for row in rows:
            ...
            for part1 in design:  # loop through once to find the cds
                if part1["name"] == from_partname:
                    ...
                    for (
                        part2
                    ) in design:  # loop through again to find the promoter
...
def main():
    ...
    parser.add_argument(
        ...
        dest="designs",
        required=True,
        help="dna_designs.csv",
        metavar="FILE",
        type=lambda x: is_valid_file(parser, x),
    )
    parser.add_argument(
        "-output", dest="output_pdf", required=True, help="output pdf filename"
    )
    parser.add_argument(
        "-reverse_char",
        dest="reverse_char",
    ...
    dna_designs = load_dna_designs(
        args.designs.name, part_info, reverse_char=cur_reverse_char
    )

    regs_info = None
    if args.regulation:
        regs_info = load_regulatory_information(
            args.regulation.name, part_info, dna_designs
        )
