...
rbs1 = {
    ...
    "end": 20,
    "type": "RBS",
    "opts": {"linewidth": 0, "color": [0.0, 0.0, 0.0]},
}
tetr = {
    "name": "tetR",
    "start": 21,
...
lac_repress = {
    ...
    "to_part": plac,
    "type": "Repression",
    "opts": {"linewidth": 1, "color": [0.38, 0.82, 0.32]},
}
gamma_repress = {
    "from_part": gamma,
    "to_part": pgamma,
    "type": "Repression",
    "opts": {"linewidth": 1, "color": [0.38, 0.65, 0.87]},
}
tet_repress = {
    "from_part": tetr,
    "to_part": ptet,
    "type": "Repression",
    "opts": {"linewidth": 1, "color": [1.00, 0.75, 0.17]},
}
...
def plot_construct(ax, t, ymtet, ymlac, ymgamma, ytet, ylac, ygamma):
    ...
    lac_repress["opts"]["linewidth"] = rescale(
        repression(ylac[tind], 2.0, 8), (0.5, 2.0)
    )
    gamma_repress["opts"]["linewidth"] = rescale(
        repression(ygamma[tind], 2.0, 8), (0.5, 2.0)
    )
    tet_repress["opts"]["linewidth"] = rescale(
        repression(ytet[tind], 2.0, 8), (0.5, 2.0)
    )
    dnaplotlib.plot_sbol_designs(
        [ax],
        [
            [
                plac,
...
def main():
    ...
    ymtet, ymlac, ymgamma, ytet, ylac, ygamma = list(
        zip(*odeint(repressilator, initial, t))
    )
    plt.close()
    ...
    plt.subplots_adjust(
        hspace=0.1, wspace=0.05, left=0.01, right=0.99, top=0.99, bottom=0.01
    )
