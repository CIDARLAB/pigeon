...
profile_fwd = dpl.load_profile_from_bed(
    "plasmid_fwd_profile.txt", "chrom1", [0, 22953]
)
profile_rev = dpl.load_profile_from_bed(
    "plasmid_rev_profile.txt", "chrom1", [0, 22953]
)
...
ax_profile.fill_between(
    list(range(cur_region[0], cur_region[1])),
    profile_fwd[cur_region[0] : cur_region[1]],
    np.zeros(cur_region[1] - cur_region[0]),
    color=(0.5, 0.5, 0.5),
...
ax_profile.fill_between(
    list(range(cur_region[0], cur_region[1])),
    np.array(profile_rev[cur_region[0] : cur_region[1]]) * -1.0,
    np.zeros(cur_region[1] - cur_region[0]),
    color=(1, 0, 0),
    edgecolor=(1, 0, 0),
    linewidth=1,
