...
from subprocess import (
    ...
    PIPE,
    STDOUT,
)  # For calling command line tools Clustal Omega and EMBOSS
...
def populate_subcomponents(parent_component):
    for ann in parent_component.annotations:
        i_start = ann.start - 1
        i_end = ann.end
        sub_seq_nucleotides = parent_component.sequence.nucleotides[
            i_start:i_end
        ]
        # ann.subComponent = sbol.DNAComponent(doc, '%s//subComponent' %ann.uri)
        ann.subcomponent.sequence = sbol.DNASequence(
            doc, "%s//Sequence" % ann.subcomponent.uri
        )
...
def remove_annotation(parent_component, deleted_ann):
    ...
    downstream_ann = deleted_ann.precedes[
        0
    ]  # Find annotation downstream of the one to be removed
...
def insert_annotation_downstream(parent_component, upstream_ann, insert_ann):
    ...
    if upstream_ann.precedes:
        downstream_ann = upstream_ann.precedes[
            0
        ]  # Assumes annotations only have one precedes relationship
        upstream_ann.precedes.remove(upstream_ann.precedes[0])
...
def insert_annotation_upstream(parent_component, insert_ann, downstream_ann):
    ...
    for i_ann, ann in enumerate(parent_component.annotations):
        ...
        if downstream_ann in ann.precedes:
            upstream_uri = (
                ann.uri
            )  # finds the annotation upstream, because it owns the precedes
...
def assemble_subcomponents(parent_component):
    parent_seq_len = 0
    for ann in parent_component.annotations:
        parent_seq_len = parent_seq_len + len(
...
def parse_fasta(fasta_str):
    ...
    entries = entries[
        1:
    ]  # Sequence has empty entry '' at the beginning of the list
    if len(entries) < 1:
...
def getSequenceAnnotationsAtBaseNo(
    parent_component, base_no, annotations_found=None
):
        ...
        if base_no >= ann.start and base_no <= ann.end:
            ...
            if ann.subcomponent and len(ann.subcomponent.annotations) > 0:
                ...
                annotations_found = annotations_found[
                    :-1
                ]  # Remove parent annotation, continue search for leaf annotations
                sub_annotations_found = getSequenceAnnotationsAtBaseNo(
                    ann.subcomponent, base_no, annotations_found
                )
                if len(sub_annotations_found) == len(
                    annotations_found
                ):  # If no leaf annotations were found at the lower level, replace the higher level annotation
...
def verify_base(ref_base, query_base):
    ...
    raise sbol.SBOLError(
        "Alignment contains unrecognized character %s or %s"
        % (ref_base, query_base)
    )
...
def flatten_subtree(dc, children_annotations=[]):
    for ann in dc.annotations:
        if ann.subcomponent:
            children_annotations = flatten_subtree(
                ann.subcomponent, children_annotations
            )
...
def calculate_identity(dna_component):
    ...
    identity = float(total_matched) / float(len(reference_seq)) * 100.0
...
def calculate_error(dna_component):
    ...
    percent_mismatched = (
        float(total_mismatched) / float(len(reference_seq)) * 100.0
    )
...
def calculate_ambiguity(dna_component):
    ...
    percent_ambiguity = (
        float(total_ambiguous_region) / float(len(reference_seq)) * 100.0
    )
...
def calculate_coverage(dna_component):
    ...
    for ann in flatten_subtree(dna_component):
        if (
            is_match(ann.subcomponent)
            or is_mutation(ann.subcomponent)
            or is_ambiguity(ann.subcomponent)
        ):
    ...
    percent_coverage = (
        float(total_covered_region) / float(len(reference_seq)) * 100.0
    )
    print("Coverage:", total_covered_region, len(reference_seq))
    return percent_coverage


parts_length_dist = {PROMOTER: 50, RBS: 15, CDS: 1000, TERMINATOR: 100}
...
def qc(design, data=None, infile=None):
    ...
    if data:
        if len(parse_fasta(data)) > 1:
            ...
            clone = data
        target_design = write_to_fasta(
            [(design.uri, design.sequence.nucleotides)]
        )
        alignment_qc = align(
            target_design + "\r\n" + clone,
            ...
            if len(alignment_coordinates) > 0:
                    ...
                    if i_alignment == 0:
                        ...
                        if not current_term == previous_term:
                            ref_start = covered_coordinates[
                                reg_start
                            ]  # Translate from alignment to design / reference coordinates
                ...
                for i_region, region in enumerate(regions):
                    ...
                    if qc_classification:
                        if (
                            qc_classification == SO_NUCLEOTIDE_MATCH
                        ):  # The reference sequence matches the query sequence
                            annotated_region = sbol.SequenceAnnotation(
                                doc,
                                "%s/MatchedSequence/SA%d"
                    ...
                    print(
                        "Adding %s to %s from %d to %d"
                        % (
                            annotated_region.uri,
                            ann.subcomponent.display_id,
                            annotated_region.start,
                            annotated_region.end,
                        )
                    )
...
def align(sequencing_data, outfile=None):
    ...
    if outfile != None:
        ...
        align_sequences = Popen(
            [
                "%s\clustalo.exe" % CLUSTAL_DIR,
                "-i",
                "-",
                "-o",
                "%s" % outfile,
...
def find_consensus(alignment):
    find_consensus = Popen(
        ["%s\cons.exe" % EMBOSS_DIR, "-filter", "-identity", "2"],
        stdin=PIPE,
        stdout=PIPE,
        stderr=STDOUT,
    )
...
def initialize_design(doc):
    n_designs = len(
        [
            ...
            if part.part_type and part.part_type == DESIGN
        ]
    )
    ...
    root.sequence = sbol.DNASequence(
        doc, "%s/Design_%d/Seq_%d" % (BASE_URI, n_designs + 1, n_designs + 1)
    )
