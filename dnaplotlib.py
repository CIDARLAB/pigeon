...
def sbol_promoter(ax, type, num, start, end, prev_end, scale, linewidth, opts):
    ...
    l2 = Line2D(
        [
            start,
            start + dir_fac * x_extent - dir_fac * (arrowhead_length * 0.5),
        ],
        [dir_fac * y_extent, dir_fac * y_extent],
        linewidth=linewidth,
        color=color,
        zorder=10 + zorder_add,
    )
    ax.add_line(l1)
    ax.add_line(l2)
    p1 = Polygon(
    ...
    )  # This is a work around for matplotlib < 1.4.0
...
def stick_figure(ax, type, num, start, end, prev_end, scale, linewidth, opts):
    ...
    if start > end:
        ...
        x1 = Line2D(
            [start, end],
            [-y_extent * 1.25, -y_extent / 1.5],
            linewidth=linewidth,
            color=color,
            zorder=12 + zorder_add,
            linestyle="-",
        )
        x2 = Line2D(
            [start, end],
            [-y_extent / 1.5, -y_extent * 1.25],
            linewidth=linewidth,
            color=color,
            zorder=12 + zorder_add,
            linestyle="-",
        ...
        dash1 = Line2D(
            [end + ((start - end) / 2.0), end + ((start - end) / 2.0)],
            [0, -y_extent / 4],
            linewidth=linewidth,
            color=color,
            zorder=8 + zorder_add,
    ...
    if start > end:
        ...
        wavy_stem_path = Path(
            vertices=[
                ...
                [wave_start + wave_bezier_dx, -(wave_height + wave_bezier_dy)],
                [
                    wave_start + wave_bezier_dx,
                    -(2 * wave_height - wave_bezier_dy),
                ],
        ...
        loop_stem_path = Path(
            vertices=[
                ...
                [
                    loop_end,
                    -(loop_offset_y + loop_height * 2 + loop_bezier_amp),
                ],
                [
                    loop_start,
                    -(loop_offset_y + loop_height * 2 + loop_bezier_amp),
                ...
                [
                    loop_end,
                    -(loop_offset_y + loop_height * 3 + loop_bezier_amp),
                ],
                [
                    loop_start,
                    -(loop_offset_y + loop_height * 3 + loop_bezier_amp),
                ...
                [
                    loop_end,
                    -(loop_offset_y + loop_height * 4 + loop_bezier_amp),
                ],
                [
                    loop_start,
                    -(loop_offset_y + loop_height * 4 + loop_bezier_amp),
...
def sbol_5_overhang(
    ...
    l_bottom = Line2D(
        [start + (x_extent / 2.0), start + x_extent],
        [-1 * y_extent, -1 * y_extent],
        linewidth=linewidth,
        color=color,
        zorder=12 + zorder_add,
...
def sbol_blunt_restriction_site(
    ax, type, num, start, end, prev_end, scale, linewidth, opts
):
...
def sbol_primer_binding_site(
    ...
    if direction == "F":
        verts = [
            (start, y_offset),
            (end, y_offset),
            (end - arrowhead_length, y_offset + y_extent),
        ]
    ...
    if opts != None and "label" in list(opts.keys()):
        if start > end:
            write_label(
                ax, opts["label"], end + ((start - end) / 2.0), opts=opts
            )
        else:
            write_label(
                ax, opts["label"], start + ((end - start) / 2.0), opts=opts
            )
...
def sbol_5_sticky_restriction_site(
    ax, type, num, start, end, prev_end, scale, linewidth, opts
):
    ...
    l1 = Line2D(
        [start + end_space, start + end_space + x_extent],
        [0, 0],
        linewidth=linewidth,
        color=color,
        zorder=12 + zorder_add,
    ...
    l1_top = Line2D(
        [start + end_space, start + end_space],
        [0, y_extent],
        linewidth=linewidth,
        color=color,
        zorder=12 + zorder_add,
    ...
    l1_bottom = Line2D(
        [start + end_space + x_extent, start + end_space + x_extent],
        [0, -y_extent],
        linewidth=linewidth,
        color=color,
        zorder=12 + zorder_add,
...
def sbol_3_sticky_restriction_site(
    ax, type, num, start, end, prev_end, scale, linewidth, opts
):
...
def sbol_signature(ax, type, num, start, end, prev_end, scale, linewidth, opts):
    ...
    if direction == "F":
        ...
        ax.add_line(lcross2)
        lsign = Line2D(
            [bot2x + indent_fac, end - indent_fac],
            [-y_extent + indent_fac, -y_extent + indent_fac],
            linewidth=linewidth,
            color=color,
            zorder=12 + zorder_add,
            linestyle=linestyle,
...
def sbol_triangle(ax, type, num, start, end, prev_end, scale, linewidth, opts):
    ...
    # Draw the CDS symbol
    p1 = Polygon(
        [(start, y_extent), (start, -y_extent), (end, 0)],
        edgecolor=edgecolor,
        facecolor=color,
        linewidth=linewidth,
        hatch=hatch,
...
def indicate(
    ax, type, num, from_part, to_part, scale, linewidth, arc_height_index, opts
):
...
def regulation(
    ...
    line_rep = Line2D(
        [end - arrowhead_length, end + arrowhead_length],
        [arcHeightEnd, arcHeightEnd],
        linewidth=linewidth,
        color=color,
        zorder=12,
    ...
    line_ind1 = Line2D(
        [end - arrowhead_length, end],
        [arcHeightEnd + indHeight, arcHeightEnd],
        linewidth=linewidth,
        color=color,
        zorder=12,
        linestyle="-",
    ...
    tall_line_toward = Line2D(
        [end, end],
        [top + arcHeightEnd, arcHeightEnd + corr],
        linewidth=linewidth,
        color=color,
        zorder=12,
        linestyle=linestyle,
    ...
    mid_line_toward = Line2D(
        [middle, middle],
        [top - midRepOffset, top + arcHeightEnd + corr],
        linewidth=linewidth,
        color=color,
        zorder=12,
        linestyle=linestyle,
    ...
    mid_line_rep = Line2D(
        [middle - arrowhead_length, middle + arrowhead_length],
        [top - midRepOffset, top - midRepOffset],
        linewidth=linewidth,
        color=color,
        zorder=12,
        linestyle="-",
    ...
    if type == "RepToRep":
        ...
        write_arc_label(
            ...
            opts["arc_label_name"],
            middle,
            top + arcHeightEnd + arcLabelOffset,
            opts=opts,
        )
    ...
    if type == "Connection":
        ...
        patch = patches.PathPatch(
            path1, facecolor="none", lw=linewidth, edgecolor=color
        )
        ax.add_patch(patch)
...
def trace_promoter_start(
    ax, type, num, start_bp, end_bp, prev_end, scale, linewidth, opts
):
    ...
    l1 = Line2D(
        [start_bp, start_bp],
        [0 + y_offset, dir_fac * y_extent + y_offset],
        linewidth=linewidth,
        color=color,
        zorder=14 + zorder_add,
    )
    l2 = Line2D(
        [
            start_bp,
            start_bp
            + dir_fac * x_extent * scale
            - dir_fac * arrowhead_length * 0.5 * scale,
        ],
        [dir_fac * y_extent + y_offset, dir_fac * y_extent + y_offset],
        linewidth=linewidth,
        color=color,
        zorder=14 + zorder_add,
    )
    ...
    p1 = Polygon(
        [
            (
                start_bp
                + dir_fac * x_extent * scale
                - dir_fac * arrowhead_length * scale,
                dir_fac * y_extent + (arrowhead_height) + y_offset,
            ),
            (
                start_bp + dir_fac * (x_extent * scale),
...
def trace_rbs(
    ...
    ax.add_line(l1)
    c1 = Ellipse(
        (start_bp, dir_fac * y_extent + y_offset),
        width=(x_extent * scale),
        height=y_extent * 0.4,
        color=color,
        zorder=14 + zorder_add,
    )
...
def trace_cds(
    ...
    p1 = Polygon(
        [
            ...
            (
                end_bp - dir_fac * arrowhead_length * scale,
                -y_extent - arrowhead_height + y_offset,
            ),
            (end_bp, 0 + y_offset),
            (
                end_bp - dir_fac * arrowhead_length * scale,
...
def trace_terminator(
    ...
    l2 = Line2D(
        [start_bp - (x_extent * scale), start_bp + (x_extent * scale)],
        [dir_fac * y_extent + y_offset, dir_fac * y_extent + y_offset],
        linewidth=linewidth,
        color=color,
        zorder=14 + zorder_add,
...
class DNARenderer:
    ...
    def __init__(
        ...
        linewidth=1.0,
        linecolor=(0, 0, 0),
        backbone_pad_left=0.0,
        backbone_pad_right=0.0,
    ):
    ...
    def renderDNA(
        ...
        for part in parts:
            ...
            if "type" in keys:
                ...
                if "renderer" in list(part.keys()):
                    # Use custom renderer
                    prev_start, prev_end = part["renderer"](
                        ax,
                        part["type"],
                        part_num,
                        part["start"],
                    ...
                    if part["type"] in list(part_renderers.keys()):
                        prev_start, prev_end = part_renderers[part["type"]](
                            ax,
                            part["type"],
                            part_num,
                            part["start"],
            ...
            for reg in regs:
                ...
                if "type" in keys and "from_part" in keys and "to_part" in keys:
                    ...
                    if reg["type"] in list(reg_renderers.keys()):
                        ...
                        ##############################################################################
                        arcstart = (
                            reg["from_part"]["start"] + reg["from_part"]["end"]
                        ) / 2
                        arcend = (
                            reg["to_part"]["start"] + reg["to_part"]["end"]
                        ) / 2
        ...
        if plot_backbone == True:
            l1 = Line2D(
                [
                    first_start - self.backbone_pad_left,
                    prev_end + self.backbone_pad_right,
                ],
                [0, 0],
...
def plot_sbol_designs(
    axes, dna_designs, regulations=None, plot_params={}, plot_names=None
):
...
def save_sbol_designs(
    filename, dna_designs, regulations=None, plot_params={}, plot_names=None
):
    """Plot SBOL designs to axes.
    ...
    max_dna_len, lims, params = plot_sbol_designs(
        axes,
        dna_designs,
...
def load_design_from_gff(
    filename, chrom, type_map=dpl_default_type_map, region=None
):
    # Load the GFF data
    gff = []
            ...
            for attrib in split_attribs:
                ...
                if len(key_value) == 2:
                    if key_value[0] == "Name":
                        ...
                        part_attribs[key_value[0]] = convert_attrib(
                            key_value[1]
                        )
            if (
                part_name != None
                and cur_chrom == chrom
                and part_type in list(type_map.keys())
            ):
                # Check feature start falls in region
                if region != None and (
                    start_bp > region[0] and start_bp < region[1]
                ):
                    gff.append(
                        [
                            part_name,
